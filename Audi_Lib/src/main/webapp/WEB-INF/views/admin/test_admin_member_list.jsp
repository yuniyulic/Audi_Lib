...
<div class="jumbotron jumbotron-fluid headJubmotron">
	<div class="container">
		...
		<p class="lead pJumbotron">테스트 - 회원 목록 조회</p>
	</div>
</div>
<hr style="margin-bottom: 30px;">

<!-- 검색 -->
<form class="form-inline my-2 my-lg-0 formRight" method="post" action="/admin/testAdminMemberList">
	<div class="search">
...
<table class="table table-hover">
	...
	<c:choose>
		<c:when test="${empty memberList }">
			<tbody>
				<tr>
					<td colspan="12"><div class="nothing">검색결과가 준재하지 않습니다.</div></td>
				</tr>
		</c:when>
		<c:otherwise>
...
<div class="paging">
	<c:if test="${paging.startPage != 1 }">
		<!-- < 표시 (한 페이지 뒤로가기) -->
		<a href="/admin/testAdminMemberList?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
	</c:if>
	<!-- startPage는 시작페이지 , endPage는 끝페이지 , 그리고 음... p 뭐야 암튼 p 변수에 집어넣는듯 -->
	<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
		<c:choose>
			...
			<c:when test="${p != paging.nowPage }">
				<a href="/admin/testAdminMemberList?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
			</c:when>
		</c:choose>
	</c:forEach>
	<c:if test="${paging.endPage != paging.lastPage }">
		<a href="/admin/testAdminMemberList?nowPage=${paging.endPage + 1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
	</c:if>
</div>
