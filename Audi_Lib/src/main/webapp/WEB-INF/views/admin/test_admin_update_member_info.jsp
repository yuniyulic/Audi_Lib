...
<link href="/resources/css/admin/admin_update_member_info.css" rel="stylesheet">
<script src="/resources/js/member/jquery.validate.js" type="text/javascript"></script>
<script src="/resources/js/admin/admin_update_member_info.js"></script>
...
<div class="jumbotron jumbotron-fluid headJubmotron">
	<div class="container">
		<h1 class="display-4 h1Jumbotron">테스트 - 회원 관리</h1>
		<p class="lead pJumbotron">테스트 - 회원 정보 수정/삭제</p>
	</div>
</div>
<hr style="margin-bottom: 30px;">

<!-- 검색 -->
<form class="form-inline my-2 my-lg-0 formRight" method="post" action="/admin/testAdminUpdateMemberInfo">
	<div class="search">
...
<form action="/admin/testAdminUpdateMemberInfo" method="post" id="${member.memberCode }">
	<table class="table">
	<colgroup>
		<col width="15%">
		<col width="19%">
...
<div class="paging">
	<c:if test="${paging.startPage != 1 }">
		<a href="/admin/testAdminUpdateMemberInfo?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
	</c:if>
	<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
		<c:choose>
			...
			<c:when test="${p != paging.nowPage }">
				<a href="/admin/testAdminUpdateMemberInfo?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
			</c:when>
		</c:choose>
	</c:forEach>
	<c:if test="${paging.endPage != paging.lastPage }">
		<a href="/admin/testAdminUpdateMemberInfo?nowPage=${paging.endPage + 1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
	</c:if>
</div>
